...
<body class="bg-gradient-to-br from-green-50 to-green-100 min-h-screen flex items-center justify-center p-6">
    <div class="w-full max-w-2xl bg-white rounded-2xl shadow-lg overflow-hidden">
        ...
        <div class="bg-green-500 px-6 py-4">
            <h1 class="text-2xl font-bold text-white text-center">Edit Data
                Siswa</h1>
        </div>
        <!-- Form -->
        <form action="{{ url('students/' . $student->id) }}" method="POST" class="p-6 space-y-5">
            @csrf
            @method('PUT')
            <!-- NIS -->
            <div>
            ...
            <div>
                ...
                <textarea name="alamat" id="alamat" rows="3"
                    class="w-full p-3 border rounded-lg focus:ring-2 focus:ring-green-400 focus:border-green-400">{{ $student->alamat }}</textarea>
            </div>
            <!-- Email -->
            <div>
