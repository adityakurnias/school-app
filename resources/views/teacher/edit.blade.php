...
<body class="bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center min-h-screen p-4 font-sans">
    <div class="container mx-auto max-w-lg bg-white rounded-2xl shadow-2xl p-8">
        <h1 class="text-3xl font-extrabold text-center text-gray-800 mb-6">Edit Data Guru</h1>
        <form action="{{ url('teachers/' . $teacher->id) }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
            <div>
            ...
            <div>
                ...
                <input type="text" name="nama" id="nama" value="{{ old('nama', $teacher->nama) }}"
                    class="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500">
            </div>
            <div>
            ...
            <div>
                ...
                <input type="text" name="bidang_studi" id="bidang_studi"
                    value="{{ old('bidang_studi', $teacher->bidang_studi) }}"
                    class="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500">
            </div>
